...
function M.resolve_weapon_bonuses(self)
    ...
    local b1, b2 = self.effectiveness_bonus or 0, self.damage_bonus or 0
    local difficulty = ((b1*b1+b2*b2) ^ 0.75) + (self.difficulty or 0)
    init_identify_requirements(self, {Apts.WEAPON_IDENTIFICATION}, math.floor(difficulty))
...
function M.resolve_armour_bonuses(self)
    ...
    if self.mini_sprite then
        ...
        self.on_draw_args.frame = 0
    end

    local difficulty = (self.bonus ^ 1.5) + (self.difficulty or 0)
    init_identify_requirements(self, {Apts.ARMOUR}, math.floor(difficulty))
