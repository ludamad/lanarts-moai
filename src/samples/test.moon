...
-- setup viewport
viewport = with MOAIViewport.new()
    \setSize( Screen.w, Screen.h )
    \setScale( Stage.w, Stage.h )

-- setup Box2D world
world = with MOAIBox2DWorld.new()
    \setGravity( 0, -10 )
    \setUnitsToMeters( 1 / scale )
    \setDebugDrawFlags( MOAIBox2DWorld.DEBUG_DRAW_SHAPES + MOAIBox2DWorld.DEBUG_DRAW_JOINTS +
...
status.font\load( 'resources/LiberationMono-Regular.ttf' )
...
-- player foot sensor
footSensorHandler = ( phase, fix_a, fix_b, arbiter ) ->
    ...
    elseif phase == MOAIBox2DArbiter.END then
        ...
        if fix_b\getBody().tag == 'platform' then
            player.platform = nil
    if player.currentContactCount <= 2 then
        player.onGround = false
    else
