...
local lobby = require "networking.Lobby"
local tasks = require "networking.Tasks"

local game_entry_draw
local logo_path = "game/lanarts/LANARTS.png"
local small_font = font_cached_load(_SETTINGS.font, 10)
local large_font = font_cached_load(_SETTINGS.font, 20)
local alt_font = font_cached_load("src/modules/core/resources/fonts/MateSC-Regular.ttf", 14)
...
local function login_if_needed()
     if session_info.username ~= _SETTINGS.username then
         local credentials = lobby.guest_login(_SETTINGS.username)
...
local function loading_box_create(size)
    ...
    local loading_animation = Display.animation_create( Display.image_split(image_cached_load "game/lanarts/menus/loading_64x64.png", {64, 64}), 0.1 )
...
local function lobby_menu_create(on_back_click) 
    local menu = InstanceBox.create{ size = vector_min(_SETTINGS.window_size, {800, 600}) }
