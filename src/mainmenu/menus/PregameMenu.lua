...
local function pregame_joined_players_list_create()
    local font = font_cached_load(_SETTINGS.menu_font, 20)
    ...
    function pregame_list:draw(xy)
        local font = font_cached_load(_SETTINGS.font, 10)
        local x, y = unpack(xy)
...
local function pregame_menu_create(on_start_click)
    ...
    local logo = Sprite.image_create("game/lanarts/interface/sprites/lanarts_logo.png")
