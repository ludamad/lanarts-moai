...
local SETTINGS_FONT = font_cached_load(_SETTINGS.font, 10)
local BIG_SETTINGS_FONT = font_cached_load(_SETTINGS.menu_font, 20)
...
local function connection_toggle_create()
    ...
    function toggle:step(xy)
        ...
        if user_io.mouse_left_pressed() and mouse_over(xy, self.size) then
            _SETTINGS.connection_type = (_SETTINGS.connection_type + 1) % 3
        end
    end
    ...
    function toggle:draw(xy)
        ...
        if _SETTINGS.connection_type == Network.CLIENT then
            text = "Connect to a game"
            color = COL_MUTED_GREEN
            sprite = client_option_image
        elseif _SETTINGS.connection_type == Network.SERVER then
            text = "Host a game"
            color = COL_PALE_RED
            sprite = server_option_image
        else -- _SETTINGS.connection_type == Network.NONE
            text = "Single-player"
            color = COL_BABY_BLUE
...
local function respawn_toggle_create()
    ...
    function toggle:draw(xy)
        local sprite = _SETTINGS.regen_on_death and respawn or hardcore

        local x,y = unpack(xy)
        local w, h = unpack(self.size)

        local text = _SETTINGS.regen_on_death and "Respawn on Death" or "Hardcore (No respawn!)"
        local text_color = _SETTINGS.regen_on_death and TEXT_COLOR or COL_LIGHT_RED
        local sprite_color = _SETTINGS.regen_on_death and TEXT_COLOR or COL_LIGHT_RED
...
local function frame_action_repeat_toggle_create()
    ...
    function toggle:step(xy)
        ...
        if user_io.mouse_left_pressed() and mouseover then
    ...
    function toggle:draw(xy)
        ...
        self.font:draw( {color=COL_GREEN, origin = Display.LEFT_CENTER}, { x + 8, y + h / 2 }, 
            (_SETTINGS.frame_action_repeat+1) .. 'x'
        )
...
local function speed_toggle_create()
    ...
    function toggle:step(xy)
        -- Toggle the connection type
        if user_io.mouse_left_pressed() and mouse_over(xy, self.size) then
            _SETTINGS.time_per_step = _SETTINGS.time_per_step - 3
            if _SETTINGS.time_per_step < 10 then
                _SETTINGS.time_per_step = 19
            end
        end
    end

    function toggle:draw(xy)
        local text = "Speed: " .. speed_description(_SETTINGS.time_per_step)
   
        local alpha = 255 - (_SETTINGS.time_per_step - 10) * 20
...
local function label_button_create(params, color_formula, on_click)
    ...
    function label_button:step(xy) -- Makeshift inheritance
        ...
        if self:mouse_over(xy) and user_io.mouse_left_pressed() then 
            on_click(self, xy)
        end
...
local function center_setting_fields_create()
    ...
    local function add_fields()
        current_setting = _SETTINGS.connection_type
    ...
    function fields:step(xy) -- Makeshift inheritance
        InstanceLine.step(self, xy)
        if current_setting ~= _SETTINGS.connection_type then
            add_fields()
        end
    end
...
local function choose_class_message_create()
    ...
    function label:set_color()
        self.options.color = _SETTINGS.class_type == -1 and COL_PALE_RED or COL_INVISIBLE
    end
...
local function back_and_continue_options_create(on_back_click, on_start_click)
    ...
    function options:step(xy) -- Makeshift inheritance
        ...
        for obj, obj_xy in self:instances(xy) do
            ...
            if mouse_is_over and user_io.mouse_left_pressed() then click_handler() end
        end
    end
