local structparse = require "globals.flextypes.structparse"
local structgen = require "globals.flextypes.structgen"

local FieldTypes = require "globals.flextypes.FieldTypes"
...
function StructType:_all_subfields(subfields, offset, is_embedded)
    for f in self:all_fields() do
        local subfield = table.clone(f)
        append(subfields, subfield)
        subfield.offset = offset + f.offset
        subfield.has_alias = (is_embedded)
...
function StructType:all_subfields()
    local subfields = {}
    self:_all_subfields(subfields, 0, true)
    return values(subfields)
end
