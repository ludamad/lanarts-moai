...
mock_server = (server_port = 6112) ->
    server = mock_gamestate {gametype: 'server', :server_port}
    server\add_new_player("TestServer5000", true)
    return server

mock_client = (server_port = 6112) -> 
    client = mock_gamestate {gametype: 'client', server_ip: 'localhost', :server_port}
    return client

mock_player_network = (server_port, n_players) ->
    assert n_players >= 1
    states = {mock_server(server_port)}
    for i=1,n_players-1 
        append states, mock_client(server_port) 
    return states
...
return {
    ...
    :mock_client
    :mock_player_network
    :mock_player_network_post_connect
    :mock_action
}
